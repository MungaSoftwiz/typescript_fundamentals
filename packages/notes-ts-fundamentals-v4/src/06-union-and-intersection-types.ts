...
type OneThroughFive = 1 | 2 | 3 | 4 | 5 // cardinalty => No f members in a set
let lowNumber: OneThroughFive = 3 //✔️ Valid
// lowNumber = 8 //! 8 is not in the set

//? A set of even numbers from 1 to 9
type Evens = 2 | 4 | 6 | 8 // you create sets by creating types
...
let evenOrLowNumber = 5 as Evens | OneThroughFive; // type casting
...
const fail = ["error", new Error("Something went wrong!")] as const // want exactly this
...
// instanceof and typeof are type guards
...
// we are using first element of tuple as a indicator of
// what whole tuple value might be
if (first === "error") {
    // In this branch of your code, second is an Error
    second
} else {
...
printEven(y) //✔️ Guaranteed to be a even
printLowNumber(y) //✔️ Guaranteed to be in {1, 2, 3, 4, 5}
printEvenNumberUnder5(y) //✔️ Guaranteed to be a in {2, 4}
